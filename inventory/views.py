...
def dashboard(request):
    ...
    products = Product.objects.filter(user=request.user).select_related('category').all().order_by('-updated_at')
    categories = Category.objects.filter(user=request.user).all().order_by('name')
    ...
    all_products = list(Product.objects.filter(user=request.user).select_related('category').all())
    ...
    category_nodes = {
        ...
        for category in Category.objects.filter(user=request.user).all()
    }
    finance_category_map = {}
    expenses = list(Expense.objects.filter(user=request.user).all())
    ...
    category_breakdown = Category.objects.filter(user=request.user).annotate(product_count=Count('product')).order_by('name')
...
def global_search_suggestions(request):
    ...
    if len(query) >= 2:
        ...
        orders = PurchaseOrder.objects.filter(user=request.user).filter(
        ...
        expenses = Expense.objects.filter(user=request.user).filter(note__icontains=query).order_by("-date", "-created_at")[:3]
...
def product_list(request):
    ...
    products = Product.objects.filter(user=request.user).select_related("category").all().order_by("display_order", "name")
    categories = Category.objects.filter(user=request.user).all().order_by("name")
...
def product_add(request):
    ...
    if request.method == "POST":
        ...
        if category_id:
            form = ProductForm(initial={"category": category_id}, user=request.user)
        else:
...
def product_reorder(request):
    if request.content_type == "application/json":
        ...
        valid_category_ids = set(
            Category.objects.filter(user=request.user, id__in=category_ids).values_list("id", flat=True)
        )
        ...
        products_by_id = Product.objects.filter(user=request.user).select_related("category").in_bulk(product_ids)
        ...
        category_names = {
            ...
            for category in Category.objects.filter(user=request.user, id__in=label_category_ids)
        }
    ...
    products = list(Product.objects.filter(user=request.user).select_related("category").filter(id__in=product_ids))
...
def category_list(request):
    ...
    parent_categories = Category.objects.filter(user=request.user, parent__isnull=True).prefetch_related('subcategories').order_by('name')
...
def category_detail(request, pk):
    ...
    products = Product.objects.filter(user=request.user, category=category).order_by('display_order', 'name')

    selected_subcategory = request.GET.get('subcategory', '').strip()
    if selected_subcategory:
        products = Product.objects.filter(user=request.user, category_id=selected_subcategory).order_by('display_order', 'name')
...
def finances(request):
    ...
    products = Product.objects.filter(user=request.user).select_related("category").all().order_by("name")
    expenses = list(Expense.objects.filter(user=request.user).all())
...
def purchase_order_list(request):
    ...
    orders = PurchaseOrder.objects.filter(user=request.user).prefetch_related('items').all()
...
def purchase_order_create(request):
    ...
    if request.method == 'POST':
        ...
        formset = PurchaseOrderItemFormSet(request.POST, form_kwargs={'user': request.user})
        ...
        formset = PurchaseOrderItemFormSet(form_kwargs={'user': request.user})
...
def purchase_order_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        formset = PurchaseOrderItemFormSet(request.POST, instance=order, form_kwargs={'user': request.user})
        if form.is_valid() and formset.is_valid():
            order = form.save()
            formset.save()
        ...
        formset = PurchaseOrderItemFormSet(instance=order, form_kwargs={'user': request.user})
    return render(request, 'inventory/purchase_order_form.html', {
...
def purchase_order_receive(request, pk):
    if request.method == 'POST':
        order = get_object_or_404(PurchaseOrder, pk=pk, user=request.user, status='pending')
